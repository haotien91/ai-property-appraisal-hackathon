...
(() => {
  ...
  // TEAM-PROJECT-MERGE-M1: demoCases above is MOCK-MODE-ONLY prototype data.
  // Production mode reads the real backend case list (GET /api/cases via
  // Api.listCases()); a hardcoded list must never be the production source.
  const isProduction = ((window.APP_CONFIG || {}).MODE === 'production');
  const cases = isProduction
    ? []
    : demoCases.map(c=>({...c,name:typeof saved[c.id]==='string' && saved[c.id].trim() ? saved[c.id].slice(0,60) : c.name}));
  ...
  async function loadRealCases() {
    ...
    try {
      ...
      (d.cases || []).forEach(meta => cases.push(fromBackendCase(meta)));
    } catch (e) {
      console.error('[case-library] listCases failed:', e);
    }
    render();
  }
