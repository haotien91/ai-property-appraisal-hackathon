...
def _resolve_cjk_font_path() -> str:
    candidates: List[str] = [os.environ.get("CJK_FONT_PATH", "")]
...
def render_shulin_official_six_page_pdf(
    ...
    from export.page_layout import page_layout, comparison_page
    try:
        order, batches, _ = page_layout(table3_data_by_segment, table51_analysis, table4_analysis)
    except ValueError as exc:
        raise ShulinPdfDataUnavailableError(str(exc)) from exc
    ...
    for template in (_TABLE51_TEMPLATE_ID, _TABLE4_TEMPLATE_ID):
        for _ in range(batches):
            src = _open_verified_template(template)
            final_doc.insert_pdf(src)
            src.close()
    for segment_code in order:
        build_table3_page(overlay, table3_mapping, final_doc[page_index_for_segment[segment_code]],
                          segment_code, table3_data_by_segment[segment_code])
    for batch in range(batches):
        build_table51_page(overlay, table51_mapping, final_doc[len(order) + batch],
                           comparison_page(table51_analysis, batch))
        build_table4_page(overlay, table4_mapping, final_doc[len(order) + batches + batch],
                          comparison_page(table4_analysis, batch), case_no=case_no,
                          appraisal_base_date=appraisal_base_date,
