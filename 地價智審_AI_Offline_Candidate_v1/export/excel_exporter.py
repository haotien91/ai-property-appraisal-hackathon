...
def export_table51_excel(bundle: CaseExportBundle, out_path: str) -> None:
    ...
    original = wb[TABLE51_SHEET]
    items = sorted(bundle.table5_1.get('comparisons', []), key=lambda item: item['comparison_index'])
    sheets = [original] + [wb.copy_worksheet(original) for _ in range(max(0, (len(items) + 2) // 3 - 1))]
    for batch, ws in enumerate(sheets):
        if batch: ws.title = original.title.strip()[:25] + '-' + str(batch + 1)
        t51 = dict(bundle.table5_1, comparisons=[dict(item, comparison_index=i + 1)
            for i, item in enumerate(items[batch * 3:(batch + 1) * 3])])
...
def export_table4_excel(bundle: CaseExportBundle, out_path: str) -> None:
    ...
    original = wb[TABLE4_SHEET]
    items = sorted(bundle.table4.get('comparisons', []), key=lambda item: item['comparison_index'])
    sheets = [original] + [wb.copy_worksheet(original) for _ in range(max(0, (len(items) + 2) // 3 - 1))]
    for batch, ws in enumerate(sheets):
        if batch: ws.title = original.title.strip()[:25] + '-' + str(batch + 1)
        t4 = dict(bundle.table4, comparisons=[dict(item, comparison_index=i + 1)
            for i, item in enumerate(items[batch * 3:(batch + 1) * 3])])
...
def export_all_excel_files(bundle: CaseExportBundle, out_dir: str) -> Dict[str, str]:
    ...
    paths: Dict[str, str] = {}
    for seg in sorted(bundle.table3, key=lambda code: (code == bundle.base_segment_code, bundle.segments[code].get("comparison_index") or 0)):
