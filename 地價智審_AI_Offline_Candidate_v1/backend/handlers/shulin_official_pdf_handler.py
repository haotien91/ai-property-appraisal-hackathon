...
def get_shulin_official_pdf_if_applicable(event, context, case_no: str, meta: dict) -> Optional[Dict[str, Any]]:
    ...
    with timed_step(case_no, "get_shulin_official_pdf"):
        ...
        _, _, pages = page_layout(table3_data_by_segment, table51_analysis, table4_analysis)
        page_count = pages[-1]["page_end"]
        now = datetime.now()
        s3 = _s3_client()
        ...
        return response(200, {
            ...
            "forms": {"official_form": {"pdf_url": official_pdf_url, "title": "查估書表（區段勘查、區域因素分析、比較法估價）"}},
            "pdf_url": official_pdf_url,
            "official_pdf_url": official_pdf_url,
            "official_pdf_status": "READY",
            "official_pdf_page_count": page_count,
