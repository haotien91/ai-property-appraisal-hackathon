...
class TestNoRealToMockSilentFallback:
    def test_bedrock_provider_never_falls_back_to_mock_silently(self, env, monkeypatch):
        ...
        client = Mock()
        client.converse.side_effect = EndpointConnectionError(endpoint_url='https://example.invalid')
        repo.propose_ai_candidates(case_no, package_id, provider=BedrockSemanticRuleMappingProvider(client),
                                  known_regional_factors=known_reg, known_individual_factors=known_ind)
        after = repo.get_package(case_no, package_id)
        results = after.metadata['ai_semantic_candidates']
        assert results
        assert all(item['provider'] == 'BedrockSemanticRuleMappingProvider' and item['status'] == 'PROVIDER_UNAVAILABLE'
                   for item in results.values())

    def test_handler_selecting_bedrock_mode_records_unavailable_not_mock_result(self, env, monkeypatch):
        ...
        client = Mock()
        client.converse.side_effect = EndpointConnectionError(endpoint_url='https://example.invalid')
        monkeypatch.setattr(evaluation_standard, 'BedrockSemanticRuleMappingProvider',
                            lambda: BedrockSemanticRuleMappingProvider(client))
        ...
        assert "PROVIDER_UNAVAILABLE" in resp["body"]
